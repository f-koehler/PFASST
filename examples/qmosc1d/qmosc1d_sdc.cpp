...
using pfasst::examples::qmosc1d::QmOsc1d;

typedef VectorEncapsulation<double, std::complex<double>>                        EncapType;
typedef QmOsc1d<pfasst::sweeper_traits<typename EncapType::traits>>              SweeperType;
  ...
  namespace examples
  {
    namespace qmosc1d
    {
                   ...
                   const double& t_0, const double& dt, const double& t_end, const size_t& niter)
      {
        ...
        sweeper->quadrature() = quadrature_factory<std::complex<double>>(nnodes, quad_type);
...
int main(int argc, char** argv)
{
  ...
  pfasst::examples::qmosc1d::run_sdc(ndofs, nnodes, quad_type, t_0, dt, t_end, niter);
}
